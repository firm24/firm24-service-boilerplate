...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  async connect(url?: string): Promise<mongoose.Connection> {
    ...
    try {
      const connection = await mongoose.createConnection(key, { useNewUrlParser: true, useFindAndModify: false });
  ...
  dropDatabase(url: string): Promise<any> {
    return this.connections[url].dropDatabase();
  }
}
